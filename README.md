...
- with the broad-parity expansion now started by adding post-core scene-family cases, utility-helper cases for `unitFrequencyList`, `Energy2Quanta`, and `blackbody` energy/quanta, plus initial metrics-family cases for `ieXYZFromEnergy`, `xyz2luv`, `ieXYZ2LAB`, `deltaEab` (1976), and `metricsSPD` angle/CIELAB
