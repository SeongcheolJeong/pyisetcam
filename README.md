...
- and now also headless `FOTParams(...)`, `gaborP(...)`, `ieCheckerboard(...)`, `MOTarget(...)`, and `sceneRamp(...)`, plus direct `sceneCreate('ramp', ...)` support for the lightweight MATLAB scene-pattern helper layer already backed by the existing frequency-orientation, harmonic, checkerboard, and ramp generators
