...
- and now also corrected the shared `sceneCreate('line ...', ...)` mean-luminance normalization plus curated Octave parity on `sceneCreate('line ee', ...)` and `sceneCreate('line ep', ...)`, covering the equal-energy and equal-photon vertical-line geometry with offset and the same `100 cd/m^2` luminance contract
