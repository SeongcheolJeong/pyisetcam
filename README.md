...
- and now also headless `scene_spatial_support(...)` / `sceneSpatialSupport(...)`, `scene_frequency_support(...)` / `sceneFrequencySupport(...)`, `scene_init_spatial(...)` / `sceneInitSpatial(...)`, and `scene_save_image(...)` / `sceneSaveImage(...)` for the legacy scene geometry/support/export helper surface built on the existing `sceneGet(...)` and `sceneShowImage(...)` paths
...
- and now also the headless metrics-window helper layer: `metrics_camera(...)` / `metricsCamera(...)`, `metrics_compute(...)` / `metricsCompute(...)`, `metrics_get(...)` / `metricsGet(...)`, `metrics_set(...)` / `metricsSet(...)`, `metrics_description(...)` / `metricsDescription(...)`, `metrics_get_vci_pair(...)` / `metricsGetVciPair(...)`, `metrics_masked_error(...)` / `metricsMaskedError(...)`, `metrics_show_image(...)` / `metricsShowImage(...)`, `metrics_show_metric(...)` / `metricsShowMetric(...)`, `metrics_save_image(...)` / `metricsSaveImage(...)`, and `metrics_save_data(...)` / `metricsSaveData(...)`, matching the older MATLAB metrics GUI helper surface with headless payload returns
