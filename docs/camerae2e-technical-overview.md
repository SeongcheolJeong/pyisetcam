...
| RAW data factory | `camerae2e_dataset_export(...)`, `camerae2e_dataset_export_from_optimization(...)`, `camerae2e_dataset_export_adas_kitti_demo(...)`, `camerae2e_dataset_export_camera_spec_variants(...)`, `camerae2e_dataset_validate(...)` | perception 학습용 deterministic RAW NPZ, optimization best/top/Pareto case export, ADAS/KITTI YOLO demo export, KITTI-style scene의 target camera-spec proxy re-capture, parameter-lineage evidence, split, checksum, RGB preview, labels JSON, metadata JSONL, manifest export/validation |
