...
| Parameter optimization | `camerae2e_parameter_space_catalog(...)`, `camerae2e_optimize_camera_parameters(...)`, `camerae2e_optimize_parameters(...)`, `camerae2e_pareto_front(...)`, `camerae2e_optimization_report(...)` | `sensor.integration_time`, `sensor.analog_gain`, `optics.fnumber`, `ip.demosaic_method`, HW ISP delay 같은 dot-path camera parameter를 preset catalog 또는 caller space로 자동 grid search하고 FACA metric objective/constraint, Pareto front, selected scenario를 산출 |
