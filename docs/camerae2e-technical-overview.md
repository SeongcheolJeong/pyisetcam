...
| Parameter optimization | `camerae2e_parameter_space_catalog(...)`, `camerae2e_optimization_config_catalog(...)`, `camerae2e_parameter_space_validate(...)`, `camerae2e_optimize_camera_parameters(...)`, `camerae2e_optimize_parameters(...)`, `camerae2e_pareto_front(...)`, `camerae2e_optimization_report(...)` | `sensor.integration_time`, `sensor.analog_gain`, `optics.fnumber`, `ip.demosaic_method`, HW ISP delay 같은 dot-path camera parameter를 preset catalog 또는 caller space로 자동 grid search하고, configure target 목록/validation, FACA metric objective/constraint, Pareto front, selected scenario를 산출 |
| RAW data factory | `camerae2e_dataset_export(...)`, `camerae2e_dataset_export_from_optimization(...)`, `camerae2e_dataset_export_adas_kitti_demo(...)`, `camerae2e_dataset_export_camera_spec_variants(...)`, `camerae2e_dataset_validate(...)` | perception 학습용 deterministic RAW NPZ, optimization best/top/Pareto case export, ADAS/KITTI YOLO demo export, KITTI-style scene의 focal-ratio pinhole crop/resize camera-spec proxy re-capture, parameter-lineage evidence, split, checksum, RGB preview, labels JSON, metadata JSONL, manifest export/validation |
