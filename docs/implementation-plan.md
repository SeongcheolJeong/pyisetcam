...
- An initial optional `SessionContext` compatibility layer is now in place, with session-aware create/compute entry points, session-aware `camera_set(...)` / `ip_set(...)` replacement flows, and MATLAB-style session helper aliases for registering and retrieving current scene/OI/sensor/IP/camera objects.
