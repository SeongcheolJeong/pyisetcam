...
- An initial optional `SessionContext` compatibility layer is now in place, with session-aware create/compute entry points, session-aware `camera_set(...)` / `ip_set(...)` replacement flows, MATLAB-style `vcReplaceObject` / `vcReplaceAndSelectObject` replacement helpers, object counting/listing/name helpers, MATLAB-style `vcGetSelectedObject` / `vcGetObject` pair-return semantics, delete helpers with slot renumbering plus `vcDeleteSomeObjects`, bulk object-list maintenance via `vcSetObjects`, slot-allocation helpers via `vcNewObjectValue`, add-and-select plus name-generation helpers via `vcAddAndSelectObject` / `vcNewObjectName`, nonpositive-selection clearing through `vcSetSelectedObject`, object-type reporting via `vcGetObjectType`, the matching `ieAddObject` / `ieDeleteObject` / `ieGetObject` / `ieGetSelectedObject` / `ieReplaceObject` / `ieSelectObject` wrappers including nested `optics` / `pixel` / `ipdisplay` getter semantics, and a minimal `ieInitSession` / `ieSessionGet` / `ieSessionSet` layer for session metadata, preferences, GUI slots, selected-object queries, and image-size/GPU flags.
